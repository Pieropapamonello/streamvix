...
export class Cb01Provider {
    ...
    private cache = new Map<string, EmbedCacheEntry>();
    ...
    private async movieFlow(imdbId: string): Promise<FlowResult[] | null> {
        ...
        let searchHtml: string; try { searchHtml = await this.fetch(searchUrl); } catch (e) { warn('movie search fetch fail', searchUrl, String(e)); return null; }
    ...
    private async resolveToMixdrop(raw: string, pageHtml: string): Promise<string | null> {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (typeof v === 'string') {
                        ...
                        const direct = v.trim();
                        // 🟢 GENERIC: accept any ID-like url or just any url
                        if (/^https?:\/\//i.test(direct)) {
                            link = direct;
                            log('stayonline ajax direct url', { link });
                        } else {
                            // Try to extract http link
                            const mm = direct.match(/https?:\/\/[^"'<>\s]+/i);
                            if (mm) { link = mm[0]; log('stayonline ajax extracted url', { link }); }
                        }
                    }
                } else {
                ...
                // If link is still pointing to stayonline (bypass failed or returned stayonline link itself), try fallback page scrape
                if (/stayonline\./i.test(link)) {
                    ...
                    try {
                        ...
                        if (pg.ok) {
                            ...
                            // Find any URL that is NOT stayonline
                            const urls = txt.match(/https?:\/\/[^"'<>\s]+/g);
                            if (urls) {
                                // Prefer urls with /e/ or /v/ as they are likely embeds, otherwise take first non-stayonline
                                const candidates = urls.filter(u => !u.includes('stayonline.pro'));
                                if (candidates.length > 0) {
                                    // Heuristic: prefer one that looks like an embed
                                    const best = candidates.find(u => /\/[ev]\//.test(u)) || candidates[0];
                                    link = best;
                                    log('stayonline page fallback chosen', { link });
                                }
                            }
                        }
                    } catch { }
        ...
        // 🟢 REMOVED strict /mixdrop/ check. Accept whatever we have if it looks like a URL.
